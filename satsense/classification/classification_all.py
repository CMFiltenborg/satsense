...
for test_image, train_images in cv_train_test_split_images(images):
    ...
    feature_set = FeatureSet()
    for feature_name, feature_scales in feature_names:
        curr_feature_scales = tuple((fc, fc) for fc in feature_scales)
    ...
    results_path = '{root}/results/all/{fs}'.format(root=get_project_root(), fs=str(feature_set))
    ...
    for classifier_name, classifier in generate_classifiers():
        ...
        results_row = pd.Series({
            ...
            'train_non-slum_examples': y_train[y_train == 0].shape[0],
            'train_slum_examples': y_train[y_train == 1].shape[0],
        ...
        results_frame.to_csv('{}/{}_df_optimization.csv'.format(df_path, str(feature_set)))
